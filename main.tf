...
resource "snowflake_schema" "tf_db2_tf_schema2" {
  name                = "RELEASE"
  database            = snowflake_database.tf_db2.name
...
resource "snowflake_user" "tf_user" {
    ...
    default_namespace = "${snowflake_database.tf_db2.name}.${snowflake_schema.tf_db2_tf_schema2.fully_qualified_name}"
...
resource "snowflake_grant_privileges_to_account_role" "grant_usage_tf_db_tf_schema_to_tf_role" {
    ...
    account_role_name = snowflake_account_role.tf_role.name
    on_schema {
        schema_name = snowflake_schema.tf_db2_tf_schema2.fully_qualified_name
  }
}
...
resource "snowflake_grant_privileges_to_account_role" "grant_all_tables" {
    ...
    on_schema_object {
        all {
            ...
            in_schema          = snowflake_schema.tf_db2_tf_schema2.fully_qualified_name
    }
  }
}
